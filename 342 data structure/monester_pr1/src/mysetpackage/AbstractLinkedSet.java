...
/**
 * Abstract class for linked-base set.
 * 
 * @author richardyang
 * @version 1.19
 * @param <Type> is of any object type
 */
public abstract class AbstractLinkedSet<Type> implements MySet<Type> {
    ...
    public void remove(Type value) {
    	if (front != null) {				    // if the list is not empty
    		if (value.equals(front.data)) {                 // if value in the first node
    			...
    			while (prior.next != front) {               // look ahead one node

    				if (prior.next.data.equals(value)) {   // found it
    					prior.next = prior.next.next;
    					if (prior.next == front)            // if deleting the last node
    						back = prior;
    				} //endif
    				prior = prior.next;
    			} //endwhile
    ...
    public void removeAtIndex(int index) {
        ...
        if (index == 0) {		// special case for the first node
            if (front == back) {		// if this is also the only node
            		front = null;
            		back = null;
            } else {
            		front = front.next;
            ...
            if (prior.next == front)		     // if it is the last node
                back = prior;
        }
    }
    ...
    private class LinkedIterator implements Iterator<Type> {
        ...
        public void remove() {
            ...
            AbstractLinkedSet.this.remove(prior.data);
            removeOK = false;
        }
    }
}
